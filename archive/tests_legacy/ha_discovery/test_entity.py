...
def test_sensor_initialization(sample_sensor_config, mock_config, mock_device):
    ...
    sensor = Sensor(config=mock_config, device=mock_device, **sample_sensor_config)
...
def test_get_config_payload(sample_sensor_config, mock_config, mock_device):
    ...
    expected_keys = [*sample_sensor_config.keys(), "device", "object_id"]
    ...
    assert (
        payload_dict["unique_id"]
        == f"twickenham_events_{sample_sensor_config['unique_id']}"
    )
