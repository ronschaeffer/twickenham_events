...
def main():
    ...
    if error_log:
        print(
            f"Found {len(error_log)} parsing errors. Details in {errors_path}")
    ...
    with open(upcoming_events_path, 'w') as f:
        json.dump({'last_updated': timestamp,
                  'events': summarized_events}, f, indent=4)
    print(
        ...
        try:
            with MQTTPublisher(
                ...
                client_id=config.get(
                    'mqtt.client_id', 'twickenham_event_publisher'),
                ...
                # Publish event data
                process_and_publish_events(
                    summarized_events, publisher, config)
